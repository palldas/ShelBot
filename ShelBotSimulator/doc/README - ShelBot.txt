...
* Internal Credit
   * Ruchi
      * Worked on SimulatorWindow, ProgrammingWindow
         * Added various buttons and options
      * Created IntroWindow 
         * Added the loading window which waits for our resources to be loaded
      * Responsible for TextToSpeech and Analyzer
         * Worked on TextToSpeech and Analyzed so the robot code could work as well as have the option to talk
   * Pallavi
      * Responsible for simulator code (code blocks and drag and drop interface)
         * Drawing of the blocks, shadows and snapping for certain blocks
      * Responsible for ProgrammingWindowBlocks (Block, Operators, Events, Control, Motion, Sound)
         * The way the blocks run and do what they’re supposed to do
   * Laksh
      * Responsible for robot code and client server integration with simulator along with opencv if time allows
